...
layui.use(['jquery', 'excel', 'form', 'layer', 'laydate'], function () {
    ...
    form.on('submit(toolbarDemo)', function () {
        ...
        $.ajax({
            url: '/ExportTeacherListHttpServlet',
            type: 'post',
            dataType: 'json',
            ...
            success: function (data) {
                ...
                // 2. 数组头部新增表头
                dt.unshift({tId: '工号', tName: '姓名', tSex: '性别', tCollege: '学院', tTitle: '职称'});
            ...
            error: function () {
                //console.log(data);
                setTimeout(function () {
                    window.location.href = '/home/teacher_list';
                }, 2000);
            }
        });
    });
...
$.ajax({
    ...
    success: function (jdata) {
        ...
        var cnt = jdata.count;
        console.log("cnt = " + cnt);
        var teacher_count_html = '共有数据：' + cnt + ' 条';
        $('#teacherCount').html(teacher_count_html);

        // 显示管理员列表
        var ls_data = jdata.list;
        if (ls_data.length > 0) {
            console.log(ls_data);
            var teacher_list_html = '<thead>\n' +
                '        <tr>\n' +
                '            <th>工号</th>\n' +
                '            <th>姓名</th>\n' +
                '            <th>性别</th>\n' +
            ...
            ls_data.map(function (item, index) {
                teacher_list_html += '        <tbody>\n' +
                    '        <tr>\n' +
                    '            <td id="studentId">' + item.tId + '</td>\n' +
                    '            <td>' + item.tName + '</td>\n' +
                    '            <td>' + item.tSex + '</td>\n' +
                    '            <td>' + item.tCollege + '</td>\n' +
                    '            <td>' + item.tTitle + '</td>\n' +
                    '            <td class="td-manage">\n' +
                    '                <a title="编辑" onClick=\'javascript:window.open("/UpdateTeacherServlet?id=' + item.tId + '","","width=400,height=270,top=400,left=600,toolbar=no, status=no, menubar=no, resizable=no, scrollbars=no");return false;\'>\n' +
                    '                    <i class="layui-icon">&#xe642;</i>\n' +
                    '                </a>\n' +
                    '                <a onclick="delTeacher(this)" href="/OptTeacherHttpServlet?id=' + item.tId + '&opt=delete">\n' +
                    '                    <i class="layui-icon">&#xe640;</i>\n' +
                    '                </a>\n' +
                    '            </td>\n' +
                    '        </tr>\n' +
            ...
            $('#teacherList').html(teacher_list_html);
        }
    }
})
